...
class GAT(nn.Module):
    def __init__(self, in_dim, embed_dim, num_layers=2, **kwargs):
        super().__init__()
        # We don't use edge features for line graph nodes to keep it simple
        self.conv1 = GATv2Conv(in_dim, embed_dim)
        self.layers = nn.ModuleList()
        for _ in range(num_layers - 1):
            self.layers.append(GATv2Conv(embed_dim, embed_dim))
